...
// ---------------------------------------------------------------------------
// A folha pessoal não leva dinheiro
// ---------------------------------------------------------------------------

const folhaPessoal = () => corpoDaFolha({
  doGrupo: false, mes: "2026-08", nomeMes: "agosto", ano: 2026,
  diasNoMes: 31, primeiroDiaSemana: 6, impressoEm: new Date("2026-09-01T12:00:00"),
  plantoes: [
    ...
    { data: "2026-08-11", hora_inicio: "19:00", hora_fim: "07:00", horas: 12,
      valor: 1800, situacao: "escalado", local: "FUNDHOSPAR", profissional: "Gustavo" },
  ],
}).corpo;

test("folha pessoal: nenhum real sobra em lugar nenhum", () => {
  ...
  //
  // Papel impresso não escolhe quem o lê, e a escala circula: vai para o
  // hospital, para a bolsa, para a mesa de alguém. Quem quer conferir dinheiro
  // tem o Financeiro e a Produção, que são telas com permissão.
  const corpo = folhaPessoal();
  ...
  assert.doesNotMatch(corpo, /1\.500|1\.800|3\.300/, "nem o número solto, sem o R$");
});

test("folha pessoal: o que ela existe para mostrar continua lá", () => {
  const corpo = folhaPessoal();
  assert.match(corpo, /Santa Casa/);
  assert.match(corpo, /FUNDHOSPAR/);
  assert.match(corpo, /<th>Situação<\/th>/);
  assert.match(corpo, /<th>Local<\/th>/);
  assert.match(corpo, /24h/, "as horas do mês não são dinheiro e ficam");
  assert.match(corpo, /2 plantões/);
});

test("folha do grupo: segue sem valor nenhum, como sempre", () => {
  const { corpo } = corpoDaFolha({
    doGrupo: true, mes: "2026-08", nomeMes: "agosto", ano: 2026,
    diasNoMes: 31, primeiroDiaSemana: 6, impressoEm: new Date("2026-09-01T12:00:00"),
